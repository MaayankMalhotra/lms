...
@section('content')
    ...
    <div class="px-4">
        <h1 class="section-title">Edit Course Details</h1>

        @if (session('success'))
            <div class="success">{{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ route('course.update', $courseDetail->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-grid">
                ...
                <!-- Submit Button -->
                <div class="field-container">
                    <button type="submit" class="button submit-button">Update Course Detail</button>
                </div>
            </div>
        </form>
    </div>
