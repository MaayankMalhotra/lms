                                ...
                                @if (!$assignment->has_submission)
                                    <form action="{{ route('student.assignment.submit', $assignment->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <input type="file" name="submission_file" class="text-sm text-gray-600" accept=".pdf,.doc,.docx,.zip" required>
                                            <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition-colors duration-200 text-sm font-medium">
                                                Submit Assignment
                                            </button>
                                        </div>
                                    </form>
                                @else
                                    <span class="text-sm text-gray-600 font-medium">Already Submitted</span>
                                @endif
                            </div>
                        </div>
                    </div>
