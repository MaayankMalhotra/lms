...
@if(auth()->user()->role == 1 )
            ...
            <li x-data="{ isOpen: {{ request()->routeIs('admin.internship.*') ? 'true' : 'false' }} }">
                ...
                <ul x-show="isOpen" x-collapse class="ml-6 mt-2 space-y-2 border-l-2 border-gray-300 pl-4">
                    ...
                    <li>
                        <a href="{{ route('get-internship-list') }}" class="flex items-center p-2 text-sm {{ request()->routeIs('get-internship-list') ? 'bg-[#ff9800] text-white' : 'hover:bg-[#ff9800]/20' }} rounded transition">
...
@if(auth()->user()->role == 1 )
            ...
            <li>
                ...
                <a href="{{ route('student.batch_quiz_ranking') }}" class="flex items-center p-3 {{ request()->routeIs('student.batch_quiz_ranking') ? 'bg-[#ff9800] text-white' : 'hover:bg-[#ff9800] hover:text-white' }} rounded transition">
